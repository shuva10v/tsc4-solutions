...
export class Task1 implements Contract {
    ...
    async getFindBranch(provider: ContractProvider, hash: bigint, cell: Cell): Promise<Cell> {
        const { stack } = await provider.get('find_branch_by_hash', [{ type: 'int', value: hash }, { type: 'cell', cell: cell }])
