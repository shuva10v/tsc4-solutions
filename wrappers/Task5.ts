...
export class Task5 implements Contract {
    ...
    async getFibbonacci(provider: ContractProvider, n: bigint, k: bigint): Promise<bigint[]> {
        const { stack } = await provider.get('fibonacci_sequence', [{ type: 'int', value: n }, { type: 'int', value: k }])
        const res = stack.readTuple();
        let out: bigint[] = [];
        while (res.remaining > 0) {
            out.push(res.readBigNumber());
        }
        return out;
    }
}
